    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            List<Song> songsStartWithLettera = Songs
                                                .Where(x => x.Name.ToLower().StartsWith("a"))
                                                .ToList();
            Console.WriteLine($@"{songsStartWithLettera.Count} songs start with the letter ""a"" ");
            Console.WriteLine("-----------------------------------------");


            // - how many artists end with letter 'a' (case insensitive)
            List<Artist> artistEndWithLetterA = Artists
                                                .Where(x => x.FullName[x.FullName.Length - 1] == 'a')
                                                .ToList();
            Console.WriteLine($@"{artistEndWithLetterA.Count} artist's names end with the letter a");
            Console.WriteLine("-----------------------------------------");

            // - whats the name of the song with longest duration
            string songWithLongestDuration = Songs
                                                .OrderBy(x => x.Duration)
                                                .LastOrDefault().Name;
            Console.WriteLine($"song with longest duration is {songWithLongestDuration}");
            Console.WriteLine("-----------------------------------------");
            ...
            int totalDurationOfAllSongs = Songs
                                          .Select(x => x.Duration)
                                          .Sum();
            Console.WriteLine($"Total duration of all songs is {totalDurationOfAllSongs} seconds");
            Console.WriteLine("-----------------------------------------");

            // - how many albums have Songs longer than 300 seconds
            int albumsWithSongsLongerThan300seconds = Albums
                                                        .Where(x => x.Songs
                                                            .Where(y => y.Duration > 300)
                                                            .ToList().Count > 0) //IF THERE ARE MORE THAN 1 SONG IN THE ALBUM
                                                        .ToList().Count;
            Console.WriteLine($" there are {albumsWithSongsLongerThan300seconds}  albums that have Songs longer than 300 secondsm");
            Console.WriteLine("-----------------------------------------");


            // - print the names of the artists(separated with "--"), that have more than one album of PopRock genre
            List<string> artistsMoreThanOneAlbumOfPopRock = Artists
                                                               .Where(x => x.Albums
                                                                    .Where(y => y.Genre == Genre.PopRock)
                                                                    .ToList().Count > 0)
                                                               .Select(x => x.FullName)
                                                               .ToList();
            artistsMoreThanOneAlbumOfPopRock.ForEach(artist => Console.Write($"--{artist}--"));
            Console.WriteLine();
            Console.WriteLine("-----------------------------------------");

            // - print the name of the album that has highest Average duration of a song
            Album albumWithHighestAverageDurationOfSong = Albums
                                                            .OrderBy(x => x.Songs
                                                                .Select(y => y.Duration)
                                                                .Average())
                                                            .LastOrDefault();
            Console.WriteLine(albumWithHighestAverageDurationOfSong.Name);
            Console.WriteLine("-----------------------------------------");


            // - how many characters has the song that has the shortest Duration
            byte numberOfCharsOfTheSongWithTheShortestDuration = (byte)Songs
                                                                  .OrderBy(x => x.Duration)
                                                                  .FirstOrDefault()
                                                                  .Name.Length;
            Console.WriteLine($"song that has the shortest Duration has {numberOfCharsOfTheSongWithTheShortestDuration} characters");
            Console.WriteLine("-----------------------------------------");
            ...
            Album albumWithMostSongs = Albums
                                        ...
                                        .LastOrDefault();
            Console.WriteLine($"album that has most songs is the {albumWithMostSongs.Genre} album {albumWithMostSongs.Name}");
            ...
            int longestSongDuration = Albums
                                        .OrderBy(x => x.Songs.Count)
                                        .FirstOrDefault().Songs
                                        .OrderBy(x => x.Duration)
                                        .LastOrDefault().Duration;
            Console.WriteLine("Longest song duration {0}", longestSongDuration);

            // - print the name of the album that has most songs that contain letter 'a' in the name
            var albumNameSongsWithLetterA = Albums
                                              .OrderBy(x => x.Songs
